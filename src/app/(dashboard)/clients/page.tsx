...
export default async function ClientsPage() {
  ...
  return (
    <div>
      <Header
        title="Companies"
        subtitle={`${clients.filter((c) => c.status === "ACTIVE").length} active Â· ${clients.length} total`}
        actions={
          <Link
                  ...
                  <tr key={client.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        client.clientType === "CLIENT" ? "bg-blue-50 text-blue-700"
                        : client.clientType === "PROSPECT" ? "bg-purple-50 text-purple-700"
                        : client.clientType === "PARTNER" ? "bg-teal-50 text-teal-700"
                        : client.clientType === "COMPETITION" ? "bg-orange-50 text-orange-700"
                        : client.clientType === "INTERNAL" ? "bg-gray-100 text-gray-600"
                        : "bg-gray-50 text-gray-500"
                      }`}>
                        {client.clientType ? client.clientType.charAt(0) + client.clientType.slice(1).toLowerCase() : "—"}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      <span
